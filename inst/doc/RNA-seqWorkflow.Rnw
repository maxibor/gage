...
\label{sec:intro}
In this tutorial, we describe the GAGE \citep{luo:etal:2009} /Pahview  \citep{luo:etal:2013} workflows on RNA-Seq data pathway analysis and gene-set analysis. We first cover a full workflow from reads counting, data preprocessing, gene set test, to pathway visualization \hyperref[sec:nativeWorkflow]{Section \ref*{sec:nativeWorkflow}}. It is called the native workflow, because GAGE/Pahview provides all the functionality for the high level analysis. The same workflow can be used for GO analysis \hyperref[sec:go]{Section \ref*{sec:go}}, and can have different choices of input per gene scores \hyperref[sec:genescore]{Section \ref*{sec:genescore}}. GAGE and Pahview are versatile tools. They can take the output from all the major RNA-seq analysis tools and carry on for pathway analysis or visualization. In \hyperref[sec:joint]{Section \ref*{sec:joint}}, we also describe joint pathway analysis workflows with common RNA-seq analysis tools.

This workflow just covers the most common situations, and never means to be comprehensive. Relevant issues like RNA-seq data quality assessment are not included. Although we focus on RNA-seq data here, but pathway analysis workflow remains similar for microarray, particularly step \hyperref[subsec:step3]{3}-\hyperref[subsec:step4]{4} would be the same. Please check \Rpackage{gage} and \Rpackage{pathview} vigenttes for details.
...
The pathway analysis workflow is implemented all in R/Bioconductor. You need to install the relevant packages within R if you haven't done so, you will need to work with R 3.0 and Bioconductor 2.13 or newer versions. Note that \Rpackage{gageData} provides the demo RNA-seq data and ready-to-use KEGG and GO gene set data. The installation may take a few minutes. From this  point on, we are working fully under R excpet noted otherwise. Of coiurse, you need to start R first. 
...
\label{subsec:step4}
We then visualize the gene expression perturbations in significant KEGG pathways using Pathview (\hyperref[fig:2]{Figure \ref*{fig:2}}). For more information on Pathview please check the main \Rpackage{pathview} vignette and the paper  \citep{luo:etal:2013}.
<<pathview, echo=TRUE>>=
...
\section{GO analysis}
\label{sec:go}
GAGE's capability for GO analysis has long been under-appreciated partially because we always use the term "pathway analysis" \citep{luo:etal:2009}. We have demonstrate the RNA-seq data workflow with KEGG pathway analysis above, as well as in the main \Rpackage{gage} vignette. GAGE is equally well applicable for GO analysis \citep{luo:etal:2009}. In fact, the Biological Process terms have similar definitions to KEGG pathways. GAGE analysis on Biological Process could be even more informative as it includes more comprehensive and detailed pathway/process definition. However, GO definition doesn't included molecular interactions hence not able to visulize like KEGG pathway graphs (\hyperref[fig:2]{Figure \ref*{fig:2}}).

<<goanalysis, echo=TRUE, eval=FALSE>>=
library(gageData)
data(go.sets.hs)
data(go.subs.hs)
...
First, \Rpackage{DESeq2} for differential expression analysis:
<<deseq2, echo=TRUE>>=
library(DESeq2)
...
Here we used GAGE to infer the signficant pathways. But we are not limited to these pathways. We can use Pathview to visualize RNA-seq data (\R{exp.fc} here) on all interesting pathways directly.
...
The folllowing GAGE and Pathview steps remain the same as in  \hyperref[subsec:DESeq2]{Subsection DESeq2}.
