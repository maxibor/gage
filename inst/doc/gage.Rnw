...
%\VignetteDepends{multtest, graph, gageData}
...
Or install without using Bioconductor, but require packages \Rpackage{graph} and \Rpackage{multtest} to be installed too:
